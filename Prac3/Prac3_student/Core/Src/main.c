...
#define LED_FREQ_1HZ  1000 //1000ms
#define LED_FREQ_2HZ  500 // 500ms
...
int main(void)
{
  ...
  while (1)
  {
	...
	char buffer2[16];
	sprintf(buffer2, "delay_t: %lu", delay_t);
	writeLCD(buffer2);
...
void EXTI0_1_IRQHandler(void)
{
	...
	curr_millis = HAL_GetTick();
	  if (LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_0) && (curr_millis - prev_millis > DEBOUNCE_TIME))
	  {
		  ...
		  if( delay_t == LED_FREQ_1HZ){

			  delay_t = LED_FREQ_2HZ;
		  }
		  else{
			  delay_t = LED_FREQ_1HZ;
